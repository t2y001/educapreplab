...
type Choice = { label: 'A' | 'B' | 'C'; content_json: any }; // usa Block[] si lo tipas
  ...
  return (
    <div className="space-y-3">
      {choices.map((ch) => {
        ...
        const isCorrect = correct === ch.label;
        const intent =
          selected === null
            ? 'neutral'
            : isSelected
        ...
        return (
          <button
            key={ch.label}
            onClick={() => onSelect(ch.label)}
            disabled={selected !== null}
            className={`${base} ${style} ${selected ? 'cursor-not-allowed' : 'cursor-pointer'}`}
          >
            <div className="flex items-start gap-3">
              <span className="font-bold text-primary">{ch.label})</span>
